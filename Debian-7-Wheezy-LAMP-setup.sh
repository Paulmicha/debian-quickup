...
#   tested on Debian 7 "Wheezy", Drupal & Symfony friendly
#   
#   Sources :
...
#
function usage {
  echo "Debian 7 'Wheezy' quickup v${VERSION}
Drupal & Symfony friendly LAMP server quick setup shell script for local dev
Usage :
...
#       Apache
aptitude install apache2-mpm-worker -y
a2enmod rewrite
...
aptitude install php5 php5-dev php5-cli php5-common php5-mysql php5-curl php-pear php5-gd php5-mcrypt php5-xdebug -y
...
if [ -n "$(apt-cache pkgnames php5-apc)" \
  ...
  aptitude -y install php5-apc
elif [ -n "$(apt-cache pkgnames php-apc)" ]; then
  aptitude -y install php-apc
fi
...
aptitude install php5-sqlite -y
